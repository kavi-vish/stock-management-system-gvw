...
$(document).ready(function() {
    ...
    // Reinitialize fileinput (optional image)
   $("#productImage").fileinput('destroy'); // destroy previous instance first
...
$("#productImage").fileinput({
    ...
    defaultPreviewContent: '<img src="assests/images/photo_default.png" alt="No Image" style="width:160px">',
    allowedFileExtensions: ["jpg", "jpeg", "png", "gif"],
    required: false,                    // This disables "required" validation
    initialPreviewAsData: true,         // Important for default image
    ...
    $("#submitProductForm").unbind('submit').bind('submit', function(e) {
        ...
        if (!productName) { $("#productName").after('<p class="text-danger">Product Name is required</p>').closest('.form-group').addClass('has-error'); valid = false; }
        if (!quantity)    { $("#quantity").after('<p class="text-danger">Quantity is required</p>').closest('.form-group').addClass('has-error'); valid = false; }
        if (!rate)        { $("#rate").after('<p class="text-danger">Rate is required</p>').closest('.form-group').addClass('has-error'); valid = false; }
        if (!brandName)   { $("#brandName").after('<p class="text-danger">Brand is required</p>').closest('.form-group').addClass('has-error'); valid = false; }
        if (!categoryName){ $("#categoryName").after('<p class="text-danger">Category is required</p>').closest('.form-group').addClass('has-error'); valid = false; }
        if (!productStatus){ $("#productStatus").after('<p class="text-danger">Status is required</p>').closest('.form-group').addClass('has-error'); valid = false; }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success == true) {
                        ...
                        $("#add-product-messages").html('<div class="alert alert-success alert-dismissible">' +
		...
		$("#submitProductForm").unbind('submit').bind('submit', function() {
			...
			//}	// /else
				// Image is optional — remove validation completely
            $("#productImage").closest('.form-group').removeClass('has-error').removeClass('has-success');
