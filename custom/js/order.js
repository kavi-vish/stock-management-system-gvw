		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				...
				'<td style="padding-left:20px;"">'+
					...
					'<input type="number" name="rateValue[]" id="rateValue'+count+'" autocomplete="off" class="form-control" />'+
				'</td style="padding-left:20px;">'+
				'<td style="padding-left:20px;">'+
                    '<div class="form-group">'+
                    '<input type="number" name="ratedisc[]" id="ratedisc'+count+'" onkeyup="getTotal('+count+')" autocomplete="off" class="form-control" min="0" />'+
                    '</div>'+
                '</td>'+
				'<td style="padding-left:20px;">'+
					'<div class="form-group">'+
...
function getTotal(row = null) {
	if(row) {
		var originalRate = parseFloat($("#rateValue" + row).val()) || 0;
    var qty = parseFloat($("#quantity" + row).val()) || 1;
    var discount = parseFloat($("#ratedisc" + row).val()) || 0;
    ...
    var discountedRate = originalRate * (1 - discount / 100);
    discountedRate = parseFloat(discountedRate.toFixed(2));

    // SHOW DISCOUNTED RATE
    $("#discount" + row).val(discountedRate);

    // Calculate total
    var total = discountedRate * qty;
